...
function createState() {
  ...
  const runHandlers = handlers => {
    handlers.forEach(handler => {
      try { handler(); } catch (error) { console.warn('Falha em handler do lifecycle FS Fit:', error); }
    });
  };
  ...
  const dispose = () => {
    ...
    cleanups.forEach(cleanup => {
      try { cleanup(); } catch (error) { console.warn('Falha ao limpar recurso do lifecycle FS Fit:', error); }
    });
    cleanups.clear();
    pauseHandlers.clear();
