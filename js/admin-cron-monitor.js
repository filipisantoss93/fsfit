...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    @media(max-width:760px){.admin-cron-monitor-head{display:grid}.admin-cron-status{justify-self:start}.admin-cron-grid{grid-template-columns:1fr 1fr}}
...
setInterval(() => {
  if (!document.hidden) loadCronStatus().catch(console.warn);
}, 60000);
