...
async function loadEvolution() {
  ...
  document.querySelector('#weight-history').innerHTML = weights?.length ? weights.map(row => {
    ...
    const variation = diff == null ? 'Inicial' : `${diff > 0 ? '+' : ''}${diff.toFixed(1)} kg`;
    return `<tr><td>${formatDate(row.data_registro)}</td><td><strong>${row.peso_kg} kg</strong></td><td>${variation}</td><td>${esc(row.observacoes || '—')}</td></tr>`;
  }).join('') : '<tr><td colspan="4" class="empty">Nenhum peso registrado.</td></tr>';
}
